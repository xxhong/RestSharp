...
namespace RestSharp {
    ...
    public partial class Http {
        ...
        HttpResponse GetStyleMethodInternal(string method)
            => ExecuteRequest(
                ...
                r => {
                    ...
                    if (!CanGetWithBody()) throw new NotSupportedException($"Http verb {method} does not support body");
        ...
        void WriteRequestBody(WebRequest webRequest) {
            if (HasBody || HasFiles || AlwaysMultipartFormData) webRequest.ContentLength = CalculateContentLength();
            ...
            else if (RequestBody != null) requestStream.WriteString(RequestBody, Encoding);
        }
        ...
        protected virtual HttpWebRequest ConfigureWebRequest(string method, Uri url) {
            ...
            if (!HasFiles && !AlwaysMultipartFormData && method != "GET") webRequest.ContentLength = 0;

            if (Credentials != null) webRequest.Credentials = Credentials;

            if (UserAgent.HasValue()) webRequest.UserAgent = UserAgent;

            if (ClientCertificates != null) webRequest.ClientCertificates.AddRange(ClientCertificates);
            ...
            if (ReadWriteTimeout != 0) webRequest.ReadWriteTimeout = ReadWriteTimeout;

            webRequest.Proxy = Proxy;

            if (CachePolicy != null) webRequest.CachePolicy = CachePolicy;

            webRequest.AllowAutoRedirect = FollowRedirects;

            if (FollowRedirects && MaxRedirects.HasValue) webRequest.MaximumAutomaticRedirections = MaxRedirects.Value;
